...
- `GET /api/chats/{chat_id}/memory-candidates` - список предложений в память для чата.
- `POST /api/chats/{chat_id}/memory-candidates/analyze` - проанализировать чат и создать новые предложения.
- `PATCH /api/chats/{chat_id}/memory-candidates/{candidate_id}` - принять или отклонить предложение.
- `DELETE /api/chats/{chat_id}/memory-candidates/{candidate_id}` - удалить предложение.
...
- Чат можно отдельно анализировать на кандидатов в память, а затем принять или отклонить их из UI.
...
- Web-поиск поддерживает три режима: `off`, `auto` и `always`.
- `off` используется по умолчанию для новых пользователей.
- Выбранный `web_search_mode` хранится на сервере отдельно для каждого пользователя и сохраняется после перезагрузки.
- `always` включает одноразовый принудительный поиск для текущего сообщения, не меняя смысл самого пользовательского запроса.
- `auto` использует локальную детерминированную политику без отдельного LLM-вызова.
- В DuckDuckGo уходит только текущий пользовательский запрос, без передачи всей истории.
- Результаты веб-поиска считаются недоверенным контекстом: они попадают только в подсказку для ответа и не пишутся в `memory_items` или `context_summary`.
- Перед вызовом Ollama сервер применяет мягкий бюджет к длине prompt, чтобы длинная история не раздувала запрос бесконтрольно.
- Если Ollama завершает ответ с `done_reason="length"`, это считается контролируемой ошибкой, а обрезанный `assistant`-message не помечается как завершённый.
- Короткие контекстные запросы без названного объекта могут не вызвать `auto`, поэтому иногда нужен ручной поиск или `always`.
...
На этапе 0.7.3 в system prompt также добавляется отдельный блок долговременной памяти пользователя:
